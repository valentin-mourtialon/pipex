...
/*   Updated: 2023/01/16 16:37:40 by vmourtia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_childpids(t_pipex *pipex)
{
	pipex->child_pids = malloc(sizeof(int) * pipex->cmd_nbr);
	if (!pipex->child_pids)
		return (-1);
}
...
{
	pipex->index = 0;
	pipex->cmd_nbr = ac - 3; /* At least: file1 cmd1 cmd2 cmd3 file2 ac = 6 cmd_nbr = 6 - 3 */
